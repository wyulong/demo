							...
							<div class="form-inline">
								<div class="col-md-12">
										<div class="form-group">
											<label for="exampleInputEmail2" class="control-label">角色名称:</label>
	...
	<script type="text/javascript">
		function responseHandler(data) {
			...
			for (var i = 0; i < pageData.length; i++) {
				  if (pageData[i].isSystem) {
				     pageData[i].isSystem = "是";
				 } else {
				     pageData[i].isSystem = "否";
				 }
			//	pageData[i].icon = "<i class='glyphicon  glyphicon-edit></i>";
				pageData[i].ops = '';
				pageData[i].ops+="<a class='btn btn-success btn-sm' ";
				/* if(pageData[i].isSystem){
					pageData[i].ops+=" disabled='disabled'";
				} */
				pageData[i].ops+=" id='cancel' type='button' href='${pageContext.request.contextPath}/admin/role/edit?id="+pageData[i].id+"' >编辑</a>";
			}
			return {
				"total" : data.total,
				"rows" : pageData
		...
		/* var success_fn = function(e){
			toastr.success(e.data,"提示");
			$('#listTable').bootstrapTable('refresh', {url: '${pageContext.request.contextPath}/admin/role/list.json'});  
		}; */
			...
			$('#remove').click(
					function() {
						...
						swal({
							...
							$.ax(
									'${pageContext.request.contextPath}/admin/role/del.json?ids='
		...
		var TableInit = function() {
			...
			oTableInit.Init = function() {
				$('#listTable')
						.bootstrapTable(
								{
									url : '${pageContext.request.contextPath}/admin/role/list.json', //请求后台的URL（*）
									method : 'get', //请求方式（*）
